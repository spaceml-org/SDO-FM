...
class RandomIntervalSDOMLDataModule(SDOMLDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if pd.Timedelta(self.sampling_period) - pd.Timedelta(
            self.cadence
        ) * num_frames < pd.Timedelta(self.cadence):
            raise ValueError(
                f"too many 'num_frames' ({num_frames}) for sampling_period={sampling_period} and cadence={self.cadence}"
            )
...
class RandomIntervalSDOMLDataset(SDOMLDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        first_timestamp = (
            self.aligndata.index[0]
            + pd.Timedelta(self.sampling_period)
            + pd.Timedelta("1min")
        )
        self.aligndata = self.aligndata.loc[first_timestamp:]

        self.aia_means = np.r_[
            [
                self.normalizations["AIA"][wavelength]["mean"]
                for wavelength in self.wavelengths
            ]
        ]
        self.aia_stdevs = np.r_[
            [
                self.normalizations["AIA"][wavelength]["std"]
                for wavelength in self.wavelengths
            ]
        ]

        self.hmi_means = np.r_[
            [
                self.normalizations["HMI"][component]["mean"]
                for component in self.components
            ]
        ]
        self.hmi_stdevs = np.r_[
            [
                self.normalizations["HMI"][component]["std"]
                for component in self.components
            ]
        ]
    ...
    def get_aia_image(self, idx, aligned_samples):
        ...
        aia_image = np.r_[
            [
                io.load_aia(
                    self.blosc_cache, self.aia_data, idx_row_element, self.wavelengths
                )
    ...
    def get_hmi_image(self, idx, aligned_samples):
        ...
        hmi_image = np.r_[
            [
                io.load_hmi(
                    self.blosc_cache, self.hmi_data, idx_row_element, self.components
                )
                for _, idx_row_element in aligned_samples.iterrows()
            ]
        ]
    ...
    def __getitem__(self, idx):
        ...
        sampling_df = self.aligndata_full.loc[
            item.name
            - pd.Timedelta(self.sampling_period) : item.name
            - pd.Timedelta("1sec")
        ]
        aligned_samples = pd.concat(
            [sampling_df.sample(self.num_frames - 1), pd.DataFrame([item])]
        )
        ...
        r = {"timestamps": timestamps, "image_stack": image_stack}
