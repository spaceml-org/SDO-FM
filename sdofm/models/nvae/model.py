...
class AutoEncoder(nn.Module):
    ...
    def init_encoder0(self, mult):
        num_c = int(self.num_channels_enc * mult)
        cell = nn.Sequential(
            nn.ELU(), Conv2D(num_c, num_c, kernel_size=1, bias=True), nn.ELU()
        )
        return cell
            ...
            with torch.no_grad():
                ...
                if i not in self.sr_u:
                    ...
                    self.sr_u[i] = F.normalize(
                        torch.ones(num_w, row).normal_(0, 1).to(weights[i]),
                        dim=1,
                        eps=1e-3,
                    )
                    self.sr_v[i] = F.normalize(
                        torch.ones(num_w, col).normal_(0, 1).to(weights[i]),
                        dim=1,
                        eps=1e-3,
                    )
