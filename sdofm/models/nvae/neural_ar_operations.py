...
AROPS = OrderedDict(
    [
        (
            "conv_3x3",
            lambda C, masked, zero_diag: ELUConv(
                C, C, 3, 1, 1, masked=masked, zero_diag=zero_diag
            ),
        )
    ]
)
    ...
    def __init__(
        ...
        if self.masked:
            ...
            self.mask = nn.Parameter(
                torch.from_numpy(
                    create_conv_mask(
                        kernel_size, C_in, groups, C_out, zero_diag, mirror
                    )
                ),
                requires_grad=False,
            )
...
class MixLogCDFParam(nn.Module):
    ...
    def forward(self, ftr):
        ...
        m = self.num_mix
        logit_pi, mu, log_s, log_a, b, _ = torch.split(
            out, [m, m, m, 1, 1, 1], dim=2
