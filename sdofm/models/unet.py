...
actfn_map = {"relu": nn.ReLU, "sigmoid": nn.Sigmoid}
...
class DoubleConv(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, mid_channels=None, output_act_fn="relu"
    ):

        if not output_act_fn in actfn_map.keys():
            raise ValueError(
                f"invalid activation fn {output_act_fn}, only {list(actfn_map.keys())} allowed"
            )
...
class UNet(nn.Module):
    def __init__(
        ...
        self.down4 = Down(
            512 // self.size_factor, self.channels4 // bifactor // self.size_factor
        )
        ...
        self.embeddings_layers = nn.ModuleList(
            [self.conv1x1, self.fc1, self.fc2, self.fc3, self.fc4, self.convto4]
        )

        # ----- decoder -----
        self.up1 = Up(
            self.channels4 // self.size_factor,
            128 // bifactor // self.size_factor,
            bilinear,
        )
        self.up2 = Up(
            128 // self.size_factor, 256 // bifactor // self.size_factor, bilinear
        )
        self.up3 = Up(
            256 // self.size_factor, 128 // bifactor // self.size_factor, bilinear
        )
        ...
        self.encoder_decoder_layers = nn.ModuleList(
            [self.encoder_layers, self.decoder_layers]
        )

        self.embeddings_decoder_layers = nn.ModuleList(
            [self.embeddings_layers, self.decoder_layers]
        )

    def param_checksum(self):
        sum_layers_params = lambda layers: sum(
            [sum([i.sum() for i in list(layer.parameters())]) for layer in layers]
        )

        return {
            "encoder": float(
                sum_layers_params(self.encoder_layers).detach().cpu().numpy()
            ),
            "embeddings": float(
                sum_layers_params(self.embeddings_layers).detach().cpu().numpy()
            ),
            "decoder": float(
                sum_layers_params(self.decoder_layers).detach().cpu().numpy()
            ),
        }
